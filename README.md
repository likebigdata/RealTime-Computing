...
具体启动步骤
    ...
    4). 紧接着要运行MysqlToKafka.java、KafkaToFlink.java和Flask.py三个程序。他们的启动顺序没有硬性要求，但在这里建议我们按照整体实现过程的顺序来运行代码程序：先运行MysqlToKafka.java，再运         行KafkaToFlink.java，最后运行Flask.py。
